...
=======
    ...
    def Train_Test(self, data, porcentaje_entrenamiento):
        ...
        """
        train = list()
        conjuntos = int((len(data)*porcentaje_entrenamiento)/100)
        for _ in range (conjuntos):
            indice = [random.randint(0, len(data) - 1) for _ in range(len(data))]
            train_TT = [data[i] for i in indice]

            self.fit(train_TT)

            #REalizar predicciones
            y_true = [item[0][0] for item in train_TT]
            y_pred = self.predict(train_TT)

            class_prediction = list(zip(y_true,y_pred))
            train.append(class_prediction)
        return train
